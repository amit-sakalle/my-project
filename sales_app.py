...
import logging
import traceback
import random # Added for selecting random examples
...
# --- Placeholder for CRM/Data (Expanded with more fields) ---
MOCK_CRM_DATA = {
    "alpha_corp": {
...
def get_lead_info_simple(lead_key, requested_field=None):
    ...
    Args:
        ...
        requested_field (str, optional): The specific field key (e.g., 'status', 'industry') to retrieve. Defaults to None.
    Returns:
    ...
    if requested_field:
        if requested_field in info:
            value = info[requested_field]
            if isinstance(value, list):
                value_str = ", ".join(value) if value else "N/A"
            else:
                value_str = str(value) if value else "N/A"
            formatted_response = f"The <b>{requested_field.replace('_', ' ')}</b> for <b>{display_name}</b> is: {value_str}"
            return formatted_response, value
        else:
            logging.warning(f"Requested field '{requested_field}' not found for lead '{lead_key}'")
            return f"Sorry, I don't have the field '<b>{requested_field.replace('_', ' ')}</b>' for <b>{display_name}</b>.", None
    else:
        formatted_response = (
...
def get_example_questions(num_questions=2):
    ...
    examples = []
    if not MOCK_CRM_DATA:
        return ""

    lead_keys = list(MOCK_CRM_DATA.keys())
    field_options = ["status", "contact", "industry", "value", "next step suggestion"] # Use display-friendly field names

    # Ensure we don't ask for more examples than available leads
    ...
    for key in selected_keys:
        ...
        if field == "contact":
             examples.append(f"Who is the contact for {display_name}?")
        elif field == "next step suggestion":
             examples.append(f"What is the next step for {display_name}?")
        else:
             examples.append(f"What is the {field} of {display_name}?")
        # Add a general info request example
        if len(examples) < num_questions * 2: # Add some variety
             examples.append(f"Tell me about {display_name}.")

    # Select a subset of generated examples to return
    final_examples = random.sample(examples, min(len(examples), num_questions + 1)) # Show slightly more than requested if available
...
def handle_greeting():
    ...
    examples = get_example_questions(2) # Get 2 examples
...
def process_user_input_simple(user_input):
    ...
    try:
        ...
        info_keywords = {"info", "information", "detail", "tell", "find", "show", "lookup", "about", "update", "what is", "what's", "who is", "who's"}
        field_keywords = {
            "status": ["status", "stage", "progress"],
            "contact": ["contact", "person", "who", "name"],
            "value": ["value", "deal size", "amount", "worth"],
            "last_contact": ["last contact", "last contacted", "when talk"],
            "industry": ["industry", "sector", "business type"],
            "products_interested": ["products", "interested in", "services"],
            "potential_needs": ["needs", "requirements", "pain points"],
            "next_step_suggestion": ["next step", "what next", "suggestion", "plan"]
        }
        ...
        if found_lead_key:
            for field, keywords in field_keywords.items():
                if any(kw in user_input_lower for kw in keywords):
                    ...
                    if field == "contact" and "name" in keywords and "company name" in user_input_lower:
                        continue # Skip if user asks for company name specifically
            ...
            examples = get_example_questions(1) # Get 1 example for context
        ...
        examples = get_example_questions(2) # Get 2 examples for fallback
